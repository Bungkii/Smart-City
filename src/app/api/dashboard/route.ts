...
export async function GET() {
  ...
  const alerts = devices.filter(d => deviceHealth(d) !== "normal").map(d => ({ id: d.id, deviceId: d.deviceId, name: d.name, system: d.system, health: deviceHealth(d), note: d.note || (deviceHealth(d) === "offline" ? "ไม่พบข้อมูลใหม่ตามเวลาที่กำหนด" : "โปรดตรวจสอบอุปกรณ์"), time: d.receivedAt }));
