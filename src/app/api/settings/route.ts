...
export async function GET() { return NextResponse.json({ mode: await getMode(), liveConfigured: Boolean(process.env.INGEST_TOKEN), commandConfigured: Boolean(process.env.DEVICE_COMMAND_URL && process.env.CONTROL_TOKEN), storage: process.env.DB_DRIVER === "d1" ? "Cloudflare D1" : "SQLite" }); }
export async function PATCH(request: Request) {
  ...
  const parsed = z.object({ mode: z.enum(["demo", "live"]) }).strict().safeParse(body);
